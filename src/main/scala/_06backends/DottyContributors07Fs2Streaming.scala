...
object DottyContributors07Fs2Streaming extends IOApp.Simple {
  ...
  def queryContributors(user: String, repo: String): IO[Either[String, List[Contributor]]] = {
    ...
    // The HttpClientFs2Backend can be used like the HttpClientCatsBackend (see the previous example),
    // but it can be used for streaming responses too (as shown here).
    HttpClientFs2Backend.resource[IO]().use { backend: SttpBackend[IO, Fs2Streams[IO]] =>
